...
class PixelBufferExtractor {
  ...
  func extract(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
    ...
    if checkIfNewPixelBufferShouldBeRecreated(width, height) {
      ...
      CVPixelBufferCreate(
        kCFAllocatorDefault, width, height, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, attributes as CFDictionary?, &newPixelBufferRef)
    }
    guard let newPixelBuffer = newPixelBufferRef else { return nil }

    guard let srcLumaTexture = createTextureFromPixelBuffer(pixelBuffer, planeIndex: 0, format: .r8Unorm) else { return nil }
    guard let srcChromaTexture = createTextureFromPixelBuffer(pixelBuffer, planeIndex: 1, format: .rg8Unorm) else { return nil }
    guard let dstLumaTexture = createTextureFromPixelBuffer(newPixelBuffer, planeIndex: 0, format: .r8Unorm) else { return nil }
    guard let dstChromaTexture = createTextureFromPixelBuffer(newPixelBuffer, planeIndex: 1, format: .rg8Unorm) else { return nil }
  ...
  func createTextureFromPixelBuffer(_ pixelBuffer: CVPixelBuffer, planeIndex: Int, format: MTLPixelFormat) -> MTLTexture? {
    let width = CVPixelBufferGetWidthOfPlane(pixelBuffer, planeIndex)
    let height = CVPixelBufferGetHeightOfPlane(pixelBuffer, planeIndex)

    var textureRef: CVMetalTexture?
    let ret = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, textureCache, pixelBuffer, nil, format, width, height, planeIndex, &textureRef)
